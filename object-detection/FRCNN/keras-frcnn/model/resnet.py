...
from tensorflow.keras.layers import Flatten, Dense, Input, Conv2D, MaxPooling2D, Activation, TimeDistributed, ZeroPadding2D, Add, AveragePooling2D
...
def classifiers(x, input_shape, trainable=False):
...
def classifier_layers(base_layers, input_rois, num_rois, nb_classes = 21, trainable=False):

    pooling_regions = 14
    input_shape = (num_rois,14,14,1024)

    out_roi_pool = RoiPoolingConv(pooling_regions, num_rois)([base_layers, input_rois])
    out = classifiers(out_roi_pool, input_shape=input_shape, trainable=True)
